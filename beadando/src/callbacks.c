...
void keyboard(unsigned char key, int x, int y)
{
	...
	case '-':
		...
		break;
	case 'e':
		action.move_plant = TRUE;
		break;
	case 'f':
		action.move_fish = TRUE;
		break;
...
void update_camera_position(struct Camera* camera, double elapsed_time)
{
	...
	if (action.increase_light == TRUE) {
		if (light_ambient[0] < 1)
			light_ambient[0] = light_ambient[1]  += 0.01;
			glLightfv(GL_LIGHT1, GL_AMBIENT, light_ambient);
	}

	if (action.decrease_light == TRUE) {
		if (light_ambient[0] > 0 && light_ambient[1] > 0.3)
			light_ambient[0] = light_ambient[1] -= 0.01;
			glLightfv(GL_LIGHT1, GL_AMBIENT, light_ambient);
	}

	if (action.move_plant == TRUE) {
		move_plant(&world);
	}

	if (action.move_fish == TRUE) {
		move_fish(&world);
	}
}
